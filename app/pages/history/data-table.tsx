				...
				<TableRow className="border-b border-neutral-800">
					{columns.map((column, index) => (
						<TableHead key={index} className="text-neutral-400 font-normal">
							<div className="h-6 bg-purple-800/50 rounded animate-pulse w-20 mx-auto" />
						</TableHead>
					))}
				</TableRow>
			</TableHeader>
					...
					<TableRow key={rowIndex} className="border-b border-neutral-800">
						{columns.map((_, cellIndex) => (
							<TableCell key={cellIndex} className="px-4 py-3">
								<div className="h-4 bg-purple-800/50 rounded animate-pulse w-24 mx-auto" />
							</TableCell>
						))}
					</TableRow>
				))}
		...
		<div>
			...
			<div className="px-4 pb-2">
				<Input
					...
					className="max-w-sm bg-purple-800 border-neutral-700 text-white placeholder:text-neutral-400"
				/>
			</div>
						...
						<>
							<TableHeader className="sticky top-0 bg-purple-800/50 z-10">
								{table.getHeaderGroups().map((headerGroup) => (
									<TableRow
										key={headerGroup.id}
										className="border-b border-neutral-800 bg-purple-900 hover:bg-transparent"
									>
								...
								{table.getRowModel().rows.length ? (
									table.getRowModel().rows.map((row) => (
										<TableRow
											...
											className="border-b border-neutral-800 hover:bg-purple-800/50"
										>
